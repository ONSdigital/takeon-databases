...
class Contributor():
    def __init__(self,Reference="", Period="",Survey=""):
        ...
        survey_list = ['066', '067', '073', '076']
        self.Survey = random.choice(survey_list)
...
def insertContributorsToDb(sqlParameters = ''):
    query = "Insert Into [public].Contributor (Reference,Period,Survey,FormID,Status,ReceiptDate,LockedBy," \
            ...
            "Values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
    connection = SimpleMsSqlConnection(ServerName=os.getenv('HOSTNAME'), Database=os.getenv('DB_NAME'))
    connection.runSQL(query, sqlParameters)

def main():
    sqlParameters = generateInsertSqlParameters(rowsToInsert=10000, startReference='49900000001', survey='066',
                                                period='201512')
